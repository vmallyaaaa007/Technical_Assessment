  ...
  if (!showFullForm) {
    return (
      <div className="flex flex-col sm:flex-row gap-0 max-w-lg w-full">
        <div className={`relative flex-1 flex items-center border sm:rounded-l-md sm:rounded-r-none rounded-t-md sm:rounded-t-none ${isDark
          ...
          <input
            ...
            className={`w-full px-2 md:px-3 py-2.5 md:py-3 h-10 md:h-11 text-xs md:text-sm bg-transparent border-none outline-none text-gray-700 placeholder:text-gray-500`}
          />
        </div>
        <Button
  ...
  return (
    <div className="max-w-lg w-full">
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="relative flex items-center border rounded-md bg-white/80 border-white/30">
          <Mail className={`ml-3 w-4 h-4 md:w-5 md:h-5 flex-shrink-0 ${isDark ? 'text-gray-400' : 'text-gray-400'}`} />
          <input
            type="email"
            name="email"
        ...
        {message && (
          <div className={`p-3 rounded-md text-xs md:text-sm ${message.type === 'success'
